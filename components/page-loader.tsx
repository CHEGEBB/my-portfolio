...
export function PageLoader({ onComplete }: { onComplete: () => void }) {
  ...
  const [phase, setPhase]           = useState<"idle" | "loading" | "logoReveal" | "confetti" | "exit">("idle")
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const speed = current < 30 ? 3.5 : current < 75 ? 2.0 : current < 95 ? 1.2 : 4.0
      current = Math.min(current + speed * (Math.random() * 0.8 + 0.6), target)
      setProgress(Math.floor(current))

      if (current < target) {
        setTimeout(tick, current < 30 ? 10 : current < 75 ? 16 : 22)
      } else {
        setTimeout(() => {
          setPhase("logoReveal")
          setLogoPainted(true)
          setTimeout(() => setLogoRevealed(true), 300)
          setTimeout(() => setPhase("confetti"), 550)
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            transform: !logoPainted ? "translateX(-101%)" : logoRevealed ? "translateX(101%)" : "translateX(0%)",
            transition: !logoPainted ? "none" : logoRevealed

...
// ── hero.css is imported statically — Next.js/Turbopack handles it at build
// time, never at runtime, so there is ZERO insertBefore conflict.
import "./hero.css"

import { useEffect, useRef, useState, useCallback } from "react"
import { useTheme } from "@/context/theme-context"

// ── Particle sphere — pure canvas 2D ─────────────────────────────────────────
function SphereCanvas({ accent, isDark, mouse }: {
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      const cX = Math.cos(rX.v), sX = Math.sin(rX.v)
      const cY = Math.cos(rY.v), sY = Math.sin(rY.v)
      const cx = W / 2, cy = H / 2
      ...
      for (let i = 0; i < N; i++) {
        for (let j = i + 1; j < N; j++) {
          ...
          ctx.beginPath()
          ctx.strokeStyle = `rgba(${cr},${cg},${cb},${(1 - d / D) * depth * (isDark ? 0.16 : 0.1)})`
          ctx.lineWidth = 0.5
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: "var(--color-text-muted)", opacity: .4,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            fontSize: "clamp(.52rem,1vw,.64rem)",
        ...
        <h1 style={{
          ...
          fontWeight: 800, letterSpacing: "-.05em", lineHeight: .9,
          ...
          <span style={{ display: "block" }}>
            {line1.split("").map((ch, i) => (
              <AnimatedChar key={i} char={ch}
                ...
                outline={false} acc={acc} isDark={isDark} />
            ))}
          </span>
          <span style={{ display: "block" }}>
            {line2.split("").map((ch, i) => (
              <AnimatedChar key={i} char={ch}
                ...
                outline={true} acc={acc} isDark={isDark} />
            ))}
          </span>
        </h1>
        ...
        <p style={{
          ...
          fontSize: "clamp(.9rem,1.5vw,1.15rem)",

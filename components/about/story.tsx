...
export function AboutStory() {
  ...
  useEffect(() => {
    ...
    // Which story-inner elements have already been revealed
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        // Drive the track translation directly here — no separate gsap.to needed
        animation: gsap.to(track, { x: () => -getDistance(), ease: "none" }),

        // Chapter reveal: pure geometry check, no containerAnimation reference
        onUpdate(self) {
      ...
      // Intro text dissolves as scroll begins
  ...
  return (
    ...
    >
      ...
      <div style={{
        position: "absolute",
        top: "clamp(5rem,8vw,6rem)", left: "clamp(1.5rem,5vw,4rem)",
      ...
      <div ref={trackRef} style={{
        ...
        <div style={{
          ...
          <div style={{
            position: "absolute", inset: 0,
            background: isDark ? `linear-gradient(135deg,${acc}22 0%,transparent 60%)` : `linear-gradient(135deg,${acc}18 0%,transparent 60%)`,
          }} />
          <div style={{
            position: "absolute", inset: 0,
            background: isDark
              ? "linear-gradient(to top,rgba(7,7,15,0.92) 0%,rgba(7,7,15,0.5) 40%,transparent 70%)"
              : "linear-gradient(to top,rgba(240,240,250,0.9) 0%,rgba(240,240,250,0.4) 40%,transparent 70%)",
          }} />

          <div style={{ position: "relative", zIndex: 2 }}>
            <div className="intro-sub" style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "clamp(1rem,2.5vw,1.8rem)" }}>
              <div style={{ width: 24, height: "1px", background: acc }} />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(0.52rem,1vw,0.65rem)", letterSpacing: "0.18em", textTransform: "uppercase", color: acc }}>
                2022 → 2025
              </span>
            </div>

            <h2 className="intro-big" style={{
          ...
          <div style={{
            ...
            zIndex: 2, fontFamily: "var(--font-mono)", fontSize: "0.48rem",
            letterSpacing: "0.12em", color: "var(--color-text-muted)", opacity: 0.35,
          }}>04 CHAPTERS</div>
        </div>
            ...
            <div className="story-inner" style={{ opacity: 0, width: "100%" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "clamp(1.5rem,3vw,2.5rem)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.52rem", letterSpacing: "0.16em", textTransform: "uppercase", color: ch.accent }}>
                    {ch.label}
                  </span>
                </div>
              </div>
              ...
              <div style={{
                position: "absolute",
                bottom: "clamp(2rem,4vw,3.5rem)", left: "clamp(2rem,4vw,3.5rem)",
              ...
              <div style={{
                position: "absolute",
                bottom: "clamp(2rem,4vw,3.5rem)", right: "clamp(2rem,4vw,3.5rem)",
                fontFamily: "var(--font-mono)", fontSize: "0.44rem",
                letterSpacing: "0.12em", color: ch.accent, opacity: 0.4,
